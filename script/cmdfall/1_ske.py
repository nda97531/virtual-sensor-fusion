    ...
    npy_dataset = CMDFallNpyWindow(
        ...
        modal_cols={
            CMDFallConst.MODAL_SKELETON: {
                'skeleton': [c.format(kinect_id=3) for c in CMDFallConst.SELECTED_SKELETON_COLS]
            }
        }
    )
    df = npy_dataset.run()
    ...
    def concat_data_in_df(df):
        ...
        for label_idx, label_val in enumerate(label_list):
            ...
            if label_idx != 0:
                ...
                class_dict['skeleton'][label_idx - 1] = modal_dict['skeleton'][idx]
...
if __name__ == '__main__':
    ...
    train_dict = three_dicts['train']['skeleton']
    valid_dict = three_dicts['valid']['skeleton']
    test_dict = three_dicts['test']['skeleton']
    del three_dicts
    ...
    for _ in range(NUM_REPEAT):
        # create model
        backbone = ResNet1D(in_channels=30, base_filters=128, kernel_size=9, n_block=6, stride=4)
        ...
        # train and valid
        train_set = BalancedDataset(deepcopy(train_dict))
