...
def load_data(root_dir: str) -> dict:
    ...
    ucihar = UCIHARNpyWindow(
        ...
        modal_cols={
            '<(")': {
                'inertia': UCIHARConst.ACC_FEATURES + UCIHARConst.GYRO_FEATURES
            }
        }
    )
    df = ucihar.run()
    ...
    def concat_data_in_df(df):
        ...
        for label_idx, label_val in enumerate(label_list):
            ...
            class_dict['inertia'][label_idx] = modal_dict['inertia'][idx]
...
if __name__ == '__main__':
    ...
    train_dict = three_dicts['train']['inertia']
    test_dict = three_dicts['test']['inertia']
    del three_dicts
    ...
    for _ in range(NUM_REPEAT):
        # create model
        backbone = ResNet1D(in_channels=6, base_filters=64, kernel_size=9, n_block=6, stride=4)
